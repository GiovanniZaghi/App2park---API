            ...
            if(!empty($data['id_ticket'])) {
				...
				if($array['data']['cell_customer'] != null){
					...
					$args = array(
						...
						"Message" => 'Detalhes: https://www.app2park.com.br/ticket.php?id='.$array['data']['id'].'&cupom='.$array['data']['id_cupom'].' App2Park -- CUPOM : '.$array['data']['id_cupom'].' - PLACA: '.$array['data']['plate'].' - ENTRADA: '.$entrada.' - ESTACIONAMENTO : '.$array['data']['name_park'],
						"PhoneNumber" => $numero
					);
					...
					$log->createLog(null, null, $numero.' AMAZON: '.$result.' Park'.$array['data']['name_park'].' Ticket: '.$array['data']['id'], null, $now, 'API - ENVIO DE SMS TICKET.', 'API');
				}
            } else {
				$array['status'] = 'ERROR';
				$array['message'] = 'Dados não preenchidos';
